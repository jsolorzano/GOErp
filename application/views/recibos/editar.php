...
$(document).ready(function () {
	...
	// Función para quitar un registro de la tabla de Productos y recalcular los montos de la rebibo
	...
	$("#facturar").click(function (e) {
		...
		if ($('#empleado').val().trim() == '' || $('#empleado').val().trim() == 'Seleccione') {
			...
			campo6 = String($("#codrecibo").val().trim());
				...
				campos = {"cod_ad_ps":campo0, "tipo" : campo1, "concepto" : campo2, "monto" : campo3, "cantidad" : campo4, "importe" : campo5, "cod_recibo" : campo6},
				data.push(campos);
				
			})
			console.log(data);
			//~ $.post('<?php echo base_url(); ?>index.php/recibos/ControllersRecibo/guardar/',{data:data}, function (data) {
				//~ if (data.success) {
			...
			$.post('<?php echo base_url(); ?>index.php/recibos/ControllersRecibo/actualizar/', $('#form_recibo').serialize()+'&'+$.param({'data':data}), function (response) {
	...
	// Función para consultar los datos salariales del empleadoal cargar la página
	...
	$.post('<?php echo base_url(); ?>index.php/recibos/ControllersRecibo/consultar_salario/'+$("#codempleado").val(), function (response) {
		...
		//Área de totales
		$("#span_abonos").text($("#abonos").val());
		$("#span_desc").text($("#descuento").val());
		$("#span_ret_faov").text($("#ret_faov").val());
		$("#span_ret_sso").text($("#ret_sso").val());
		$("#span_sub_total").text($("#subtotal").val());
		
		$("#span_total").text($("#totalrecibo").val());
	...
	// Cargamos los datos del empleado incluyendo sus datos salariales
	 $("#hola").click(function (e) {
		...
		// Función para consultar los ajustes pendientes de las recibos del empleado
		//~ alert($("#codempleado").val());
		$.post('<?php echo base_url(); ?>index.php/recibos/ControllersRecibo/consultar_salario/'+$("#codempleado").val(), function (response) {
			var respuesta = response.split('<html>');
			//~ respuesta = $.parseJSON(respuesta[0]);
            ...
            <div class="well bs-component">
                <form id="form_recibo" class="form" method="post">
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-sm-12">
                                <legend>Rebibo de Pago (
                                <?php echo $editar->codrecibo;?>
                                )</legend>
                                <input type="hidden" id="codrecibo" name="codrecibo" value="<?php echo $editar->codrecibo;?>">
                        ...
                        <div class="row">
                            <div class="col-sm-4">
								<div style="margin-left: 0.2%;margin-bottom: -1%;" class="form-group">
									...
									<div class="input-group mar-btm">
										...
										<input type="text" style="width: 27%;" readonly="true" name="codempleado" id="codempleado" class="form-control" value="<?php echo $editar->codempleado;?>">
										<input type="text" style="width: 73%;" name="empleado" readonly="true" id="empleado" placeholder="empleado" class="form-control" value="<?php echo $editar->empleado;?>">
										<span class="input-group-btn">
                            ...
                            <div class="col-sm-2">
								<span >Salario</span>
								<div class="input-group mar-btm">
									<input type="text" data-original-title="Salario" data-toggle="tooltip" data-placement="bottom" readonly="true" name="salario" id="salario" placeholder="Salario" class="form-control add-tooltip" readonly="true" value="<?php echo $editar->salario;?>">
								</div>
                            </div>
                            <div class="col-sm-2">
                            ...
                            <div class="col-sm-2">
								...
								<div class="input-group mar-btm">
									<input type="text" style="width:100%;" data-original-title="Descuento" data-toggle="tooltip" data-placement="bottom" name="monto_salario" id="monto_salario" placeholder="0.00" class="form-control add-tooltip" readonly="true" value="<?php echo $editar->monto_salario;?>">
								</div>
                            </div>
                            </br>
                            <div class="col-sm-3">
...
-->
                             ...
                             <table style="width:100%;" border="0" align="center" cellspacing="1" id="tab_recibo" align="center"
								...
								<thead style="font-size: 14px">
									<tr class="info">
										...
										<th style='text-align: center'>Abono/Descuento</th>
								...
								<tbody >    
									<?php foreach ($listar_ad as $ad) { ?>
									<tr>
										<td style='text-align: center' id="<?php echo $ad->codrecibops;?>">
										<?php 
										if ($ad->tipo == 1){
											echo "Abono";
										}else{
										...
										<td style='text-align: center'><?php echo $ad->abono_descuento;?></td>
										<td style='text-align: center'><?php echo $ad->monto;?></td>
										<td style='text-align: center'><?php echo $ad->cantidad;?></td>
										<td style='text-align: center'><?php echo $ad->importe;?></td>
										<td style='text-align: center'><a class='quitar' id="<?php echo $ad->codrecibops;?>"><i class='glyphicon glyphicon-trash'></i></a></td>
									</tr>
									<?php }?>
								</tbody>
							</table>
                    ...
                    <div class="container-fluid" style="margin-top: 10px;">
                        <div class="row">
							<input type="hidden" id="abonos" name="abonos" placeholder="Abonos" value="<?php echo $editar->abonos;?>">
							<input type="hidden" id="descuento" name="descuento" placeholder="Descuento" value="<?php echo $editar->descuento;?>">
							<input type="hidden" id="ret_faov" name="ret_faov" placeholder="Retención FAOV" value="<?php echo $editar->ret_faov;?>">
							<input type="hidden" id="ret_sso" name="ret_sso" placeholder="Retención SSO" value="<?php echo $editar->ret_sso;?>">
							<input type="hidden" id="faov_patrono" name="faov_patrono" placeholder="Aporte FAOV" value="<?php echo $editar->faov_patrono;?>">
							<input type="hidden" id="sso_patrono" name="sso_patrono" placeholder="Aporte SSO" value="<?php echo $editar->sso_patrono;?>">
							<input type="hidden" id="subtotal" name="subtotal" placeholder="Sub-Total rebibo" value="<?php echo $editar->subtotal;?>">
							<input type="hidden" id="totalrecibo" name="totalrecibo" placeholder="Total rebibo" value="<?php echo $editar->totalrecibo;?>">
