    ...
    redirect(base_url());
        ...
        <script>
            $(document).ready(function () {
                ...
                $("select").select2();
                $('#cargo').alpha({allow: " "}); //Solo permite texto

                $('#volver').click(function () {
                    url = '<?php echo base_url()?>index.php/cargos/ControllersCargos'
                    window.location = url
                })
...
//~ 
                ...
                // Función para validar por nombre si ya existe el cargo
                $("#cargo").change(function (event) {

                    var cargo = $('#cargo').val();
                    $.post('<?php echo base_url(); ?>index.php/cargos/ControllersCargos/consultar/', $('#form_cargo').serialize(), function (response) {
						//~ alert(response[0])
						if (response[0] == 'e') {
							bootbox.alert("El cargo ya existe", function () {
                ...
                $("#registrar").click(function (e) {
                    ...
                    if (($('#cod_cargo').val().trim() == '')) {
                        bootbox.alert('Disculpe, debe colocar el Código del cargo', function() {
                        }).on('hidden.bs.modal', function (event) {
                            $('#cod_cargo').parent('div').addClass('has-error');
                            $('#cod_cargo').focus();
                        });
                    }
                    else if (($('#cargo').val().trim() == '')) {
                        bootbox.alert('Disculpe, debe colocar el nombre del cargo', function() {
                        }).on('hidden.bs.modal', function (event) {
                            $('#cargo').parent('div').addClass('has-error');
                            $('#cargo').focus();
                        });
                    ...
                    //~ }
                         ...
                         $.post('<?php echo base_url(); ?>index.php/cargos/ControllersCargos/guardar', $('#form_cargo').serialize(), function (response) {
                                ...
                                <form class="form-horizontal" id="form_cargo">
                                    <fieldset>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-5">
                                                <input class="form-control" autofocus="" value="<?php printf('CG' . "%05d", (int)$ultimo_id + (int)1); ?>" type='text' placeholder="BC00001" id="cod_cargo" maxlength="7" name="cod_cargo" readonly="true"/>
                                            </div>
                                           <div class="col-xs-1" style="font-weight:bold" >Cargo</div>
                                            <div class="col-lg-5">
                                               	<input class="form-control" type='text' placeholder="Nombre del Cargo" maxlength="100" id="cargo" name="cargo"/>
                                            </div>
